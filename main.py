...
class SupervisedTrainer(object):
    ...
    def train(self, evaluator: Evaluator=None,dbname = None) -> None:
        curve_f1 = []
        curve_epoch = []
        for epoch in range(1, self.epoch_number):
            curve_epoch.append(epoch)
            ...
            if evaluator:
                ...
                # f1, precision, recallx = tester.test()
                


        
        # plt.plot(curve_epoch, curve_f1, '--', color="#111111",  label="Training score")
        plt.plot(curve_epoch, curve_f1, color="#111111", label="Training score")

        plt.fill_between(curve_epoch, curve_f1, color="#DDDDDD")
        # plt.fill_between(curve_epoch, curve_f1, color="#DDDDDD")

        plt.title(dbname+"Dataset")
        plt.xlabel("Training Set Size"), plt.ylabel("Accuracy Score"), plt.legend(loc="best")
        plt.tight_layout()
        plt.savefig(dbname+".png")
        plt.show()
...
#         print(f"hello{config.dataset_name}")
    else:
    ...
    if train:
        ...
        trainer.train(evaluator,config.dataset_name)
    else:
